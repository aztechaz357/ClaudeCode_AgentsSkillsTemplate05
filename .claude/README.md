...
| `/journal [commands\|list]` | 共通 | **思考の記録** （外した仮説・却下案・実測のコマンドを文章と UML で残す。学習用・任意） |
...
（例: 「現在 L1、目標 L2」）と関与モード（例: 「関与モード: 伴走」）を
必ず書く。** 目標成熟度を書き忘れると最上位の基準で判定し、完璧の
積み上げが始まる —— このテンプレートで最も多い事故。関与モードを書き
忘れるとエージェントが決めてから説明し、 **ユーザーが置いてけぼりになる** 。
