# 使い方（1 枚）—— どのコマンドを打ち、誰が動くか

**このファイルは操作の早見表。** 考え方の正は `./SKILL.md`、
作るものの正は `./deliverables.md`、完了条件の正は `./maturity.md`。

## 3 行で言うと

1. **端から端まで動く骨組みを最初に作り切る**（`/skeleton`。1 回だけ）
2. **スライス 1 本ずつ、7 点セットをそろえて成熟度を 1 段上げる**（`/iterate` を繰り返す）
3. **迷ったら `/status`**（現在地が 1 画面で出る）

## いつ何を打つか

| こうしたい・こうなった | 打つもの |
|---|---|
| 新しいプロジェクトを始める | `/setup-project` → `/backlog init` |
| まだ何も動いていない | `/skeleton`（**通るまで他のことをしない**） |
| 次の 1 本を進めたい | `/iterate`（引数なしでバックログ最上段） |
| 特定のスライスを進めたい | `/iterate S03` |
| 今どこにいるか分からなくなった | `/status` |
| しばらく席を外した・生成に追いつけない | `/catchup`（決まったことを後戻りコスト順に） |
| 次に何をやるか決められない | `/backlog 次`（機械的に 1 つだけ返る） |
| 負債がたまってきた（反復 2 回分） | `/refactor` |
| 不可逆・外部公開・安全機構・データ形式が絡む | `/add-feature`（例外の経路） |
| 記録が実物とずれている気がする | `/check-docs` |
| 同じ失敗を繰り返している | `/improve-process` |
| 思いつきの小さな道具が欲しい | `/tool <説明>`（工房レーン。7 点セットは不要） |
| GitHub Issue で進捗を見せたい / やめたい | `/issue on` / `/issue off`（既定は使わない） |

**`/iterate` が既定の入口。** 他は必要になったときだけ。

## 反復 1 回の中身（`/iterate` の 8 段）

1 回 = 半日以内で 7 点セットをそろえ、成熟度を **1 段だけ** 上げる。

| 段 | 誰が | 作るもの | 読むスキル | L1 の上限 |
|---|---|---|---|---|
| 1 | `requirement-writer` | `docs/usdm/src/S##-*.html` | `usdm` | 要求 1・理由 1 行・仕様 3 条 |
| 2 | `designer` | `docs/design/S##-*.md` | `functional-design` `layered-architecture` | 構造の図 ＋ 10 行 ＋ 判断の記録 |
| 3 | `unit-tester` | 単体テスト（**Red 確認まで**） | `development-guidelines` `visual-debugging` | 1〜3 本 |
| 4 | `coder` | 実装（Green まで） | `layered-architecture` | 差分 20〜30 行で区切る |
| 5 | `integration-tester` | E2E テスト・実物の出力 | —— | 正常系 1 本 |
| 6 | `test-summarizer` | `docs/test-reports/S##-*.md` | `test-reporting` | 1 枚（実測のみ） |
| 7 | `manual-writer` | `docs/manual.md` の `## S##` 節 | `writing-conventions` | 節 1 つ |
| 8 | `implementation-validator` → `doc-syncer` | 判定・記録の同期 | `maturity.md` `steering` | —— |

**順序は動かせない。** 実装を先に書くと、要求と設計が後追いの言い訳になる。
**1 つでも次の反復へ送らない。** 送った 1 つは二度と書かれない。

## エージェントの起動（Task で委譲するとき）

プロンプトに **次の 4 つを必ず書く** 。書き忘れが事故の最大の原因。

```
1. .steering/20260807-S03-csv-filter/     ← 作業ディレクトリ
2. S03、現在 L1、目標 L2                   ← 目標成熟度（★ 省略厳禁）
3. reports/02-designer.md を読んでから     ← 前の段のレポート
4. reports/03-unit-tester.md に書き出し、応答はパスと 5 行以内の要約のみ
```

**目標成熟度を書かないと、エージェントは最上位の基準で判定する。**
その結果 L1 の反復で失敗経路を網羅し始め、半日で終わらなくなる。

規模が小さければ委譲せず自分でやってよい。ただし **その場合も同じ
レポートを残す** （誰がやったかにかかわらず、受け渡しの形は変えない）。

## 完了したと言えるか（目視で判断しない）

```
<ツール実行コマンド> .claude/tools/check_deliverables.py --slice S03   # 7 点がそろっているか
<ツール実行コマンド> .claude/tools/build_usdm.py                       # 要求と成果物の線が切れていないか
```

両方 0 で、かつ全テストが緑なら完了。どちらかが 1 なら未完了。

## 困ったとき

| 症状 | どうするか |
|---|---|
| 反復が半日で終わらない | スライスが大きすぎる。`docs/backlog.md` で割る |
| 上限（仕様 3 条・設計 10 行）を超える | 書きすぎではなくスライスが大きすぎる。割る |
| 3 回作り直しても L1 に届かない | 設計不足。`/add-feature` の厚い経路へ切り替えを相談する |
| 仕様と実装が矛盾した | **止まってユーザーに確認する**（勝手に片方へ合わせない） |
| 骨組みの E2E が赤くなった | 回帰。最優先で直す。他の作業を進めない |
| どの層に置くか分からない | `layered-architecture` の判定フローを上から順に問う |
| 契約を切るべきか分からない | 外部 I/O（ファイル・DB・通信・時刻・乱数）があるときだけ切る |
| エージェントが越権した | 起動プロンプトの目標成熟度を確認する（大抵は書き忘れ） |

## やってはいけない 5 つ

1. **7 点のうち 1 つを次の反復へ送る**（最も重い失敗）
2. 実装してから要求・設計を書き写す（順序を守る）
3. 2 段まとめて上げる（L0 → L2）
4. 手抜きを記録せずに残す（記録された手抜きだけが許される）
5. 設計書に結論だけ書く（理由・他の選択肢・メリデメを残す）

## 覚えるファイルは 3 つだけ

| ファイル | 何の正か |
|---|---|
| `docs/backlog.md` | **進捗** （成熟度・いま着手中・負債）。他の文書に進捗を書かない |
| `./deliverables.md` | **作るもの** （7 点セットの置き場所・上限・担当） |
| `./maturity.md` | **完了条件** （L1 / L2 / L3 の DoD） |

迷ったらこの 3 つに戻る。それ以外は必要になったときに読めばよい。
