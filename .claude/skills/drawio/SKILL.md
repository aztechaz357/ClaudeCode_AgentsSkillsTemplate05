...
- **セルの `style` に `fontFamily=Yu Gothic;` を必ず入れる** —— draw.io の
  既定フォントは Helvetica で、日本語のグリフを持たない。ラベルの文字は
  書き出しても消えない（SVG に原文のまま残ることを実測）が、字形は
  受け手のフォールバック任せになる。指定すると書き出しに
  `font-family: 'Yu Gothic'` が入る（実測）
