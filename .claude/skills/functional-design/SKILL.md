...
| 分量 | **入出力と例 2 行** ＋ **4 種の図（各 5 ノードまで）** ＋ 構成 10 行 ＋ 主張の表 ＋ 判断の記録 3 行 | 制限なし（仕様の充足方針まで） |
...
設計書の冒頭は **入出力の表と、正常系・異常系の例 1 つずつ** で始める。
何を入れると何が出るかが 1 行で分かって初めて、
続く 4 枚の図が何を説明しているのかが読める。

- 書式と ID（`N##` / `E##`）の規約の正は
  `agile-process/deliverables.md` の「入出力と例（正常系・異常系）」
- **要求仕様書と同じ ID が同じケースを指す** 。設計書では
  それを **型と層の言葉** に翻訳する（要求は利用者の言葉）
- **2 行を超えない。** 網羅したくなったら
  `docs/test-specs/S##-<name>.md`（テスト仕様書）に書く
- **異常系（`E1`）を空にしない。** 決まっていないなら要求の穴なので、
  設計を進めずに段1 へ差し戻す
...
- [ ] **入出力の表があり、「なし」で済ませていない**
      （入力が無いなら「引数なし・標準入力なし」と書く）
- [ ] **正常系 `N1` と異常系 `E1` が 1 行ずつある**（`check_deliverables.py`
      がテスト仕様書側で機械検証する。設計書は目で確かめる）
- [ ] **`E1` と「主張」節の反例が食い違っていない**
