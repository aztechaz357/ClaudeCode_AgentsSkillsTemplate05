...
テスト仕様: [S##](../test-specs/S##-{name}.md) ｜
ハブ: [S##](../slices/S##-{name}.md)

> この 1 枚は **毎スライスで必ず書く** （省略も後回しも不可）。
> ただし L1 では下の上限を超えない —— 超えたらスライスが大きすぎる。
> 上限: **4 種の図（1 枚あたりノード 5 個まで）** ＋ 構成 10 行 ＋
> 入出力の例 2 行 ＋ 主張の表 ＋ 判断の記録 3 行。
> **薄くするのは 1 枚あたりであって、枚数ではない**（`functional-design`）。

## 入出力

> **図より先にここを読ませる。** 何を入れると何が出るかが 1 行で分かれば、
> 以降の 4 枚の図が何を説明しているのかが分かる。

| | 内容 |
|---|---|
| **入力** | {型と形。どの入口で受けるか。例: `Path`（CSV ファイル）を `presentation.cli` が受ける} |
| **出力** | {型と形。どこへ出すか。例: `int`（行数）を標準出力へ ／ 終了コード `0`} |

### 例（正常系・異常系）

> 書式と ID の規約の正は
> `.claude/skills/agile-process/deliverables.md` の
> 「入出力と例（正常系・異常系）」。 **`N1`・`E1` は要求仕様書と
> 同じケースを指す** （こちらは型と層の言葉で書く）。
> **2 行を超えない。** 網羅は `docs/test-specs/S##-{name}.md` の仕事。

| ID | 種別 | 入力 | 期待する出力 | 備考 |
|---|---|---|---|---|
| N1 | 正常系 | {型と値。例: `Path("data.csv")`（3 行）} | {型と値。例: `3` を標準出力 ／ 終了コード `0`} | |
| E1 | 異常系 | {型と値。例: `Path("nope.csv")`（存在しない）} | {例: `入力ファイルが見つかりません: nope.csv` を標準エラー ／ 終了コード `2`} | {例外を投げるか、結果値で返すか} |

<!--
  E1 は「主張（契約式）」節の **反例** を具体値にしたもの。
  両方書いたら突き合わせる —— 食い違っていたらどちらかが嘘。
  異常系が決まっていないなら、それは要求の穴。段1 に差し戻す。
-->
