...
**🔧 設計済み・未実装**。要求仕様: [P##](../../requirements/P##-<name>.md)。
依存: [P##](P##-<name>.md)（{依存理由}。無ければ「依存なし」）。
...
要求仕様書の受け入れ条件を、どの設計要素が満たすかの対応表。

| 受け入れ条件 | 満たす設計要素 | 検証方法（単体 / 統合） |
|---|---|---|
...
> 実装フェーズ（TDD）はここをテストコードに翻訳するところから始まる。
...
| 決定 | 理由 |
|---|---|
| {採用した設計} | {なぜ。家風パターン（core 無変更・共有状態注入等）との関係} |
