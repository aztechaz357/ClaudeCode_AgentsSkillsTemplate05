...
```
<ツール実行コマンド> .claude/tools/build_arch.py <ソースルート>   # 実物の図（層と依存）
...
### 4 種の図をどこまで機械で突き合わせられるか

設計書は 4 種の図を持つ（正: `functional-design`）。逆生成できる範囲は
図ごとに違うので、 **できないものをできるふりにしない** 。

| 設計書の図 | 逆生成 | 突き合わせ方 |
|---|---|---|
| フローチャート（層と依存） | **できる** | `diff_arch.py` が集合比較して色分けする（機械） |
| クラス図 | **できる** | `build_uml.py --kind class` と目で比較（クラス名・継承・保持） |
| シーケンス図 | **一部** | `--kind sequence` は呼び出し関係だけ。 **順序は出現順であって実行順ではない** 。分岐とループは出ない |
| 状態遷移図 | **できない** | 下記の「名前の対応」で見る |

**状態遷移図を実装から起こさない理由**: 任意のコードから状態機械は
一般に復元できない。それらしい図を出すと「実装と一致している」という
**誤った確信** を与え、乖離検出が逆向きに壊れる（`build_uml.py --kind state`
は終了コード 2 で拒否する）。

代わりに **名前の対応** を見る —— 設計書の状態遷移図に書いた状態名が、
実装の列挙型・定数に存在するか。構造の一致は保証されないが、
**状態が増えた / 消えたことには気づける** 。

**逆生成した図は実装より豪華にしない。** `build_uml.py` は非公開メソッド
（`_` 始まり）と型注釈の無い属性を落とす。推測で足した要素は
「設計書に無いもの」として誤検出され、乖離が常に赤くなる。
