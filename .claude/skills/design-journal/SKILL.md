---
name: design-journal
description: 設計・問題解決の途中で考えたことを、文章と UML（Mermaid）で人が読める形に残す規約。外した仮説・却下した案・実際に打ったコマンドまで書き、AI の思考を人が真似できるようにする学習用の記録。合図（判断点で選択肢を出した・詰まって抜けた・節目の操作をした）が出たら聞かずに書く既定 ON の成果物で、8 点セットの外・完了条件の対象外。選択肢を出した直後、詰まって抜けた直後、PR やマージなど節目の操作をした直後に読み込む。
...
| 手戻り | 結果だけ残す | 順番どおりに残す |
| 量 | 3 行〜 | 1〜2 画面 |
| 義務 | **必須** （8 点セットの一部） | **既定 ON・完了条件の外** （下記） |

## 「既定 ON」と「必須」は違う（この区別が命）

| | 既定 ON（このスキル） | 必須（8 点セット） |
|---|---|---|
| 合図が出たら | **聞かずに書く** | 書く |
...
**ジャーナルは 8 点目の成果物ではない。** 8 点セットは 8 点のまま。
...
- **採用: 8 点セットの外に置き、 **既定 ON** にした**（2026-08-14 に
