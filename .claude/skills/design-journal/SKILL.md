---
name: design-journal
description: 設計・問題解決の途中で考えたことを、文章と UML（Mermaid）で人が読める形に残す規約。外した仮説・却下した案・実際に打ったコマンドまで書き、AI の思考を人が真似できるようにする学習用の記録。判断点で選択肢を出した直後、詰まって抜けた直後、PR やマージなど節目の操作をした直後に読み込む。
...
| 手戻り | 結果だけ残す | 順番どおりに残す |
| 量 | 3 行〜 | 1〜2 画面 |
| 義務 | **必須** （7 点セットの一部） | **任意** （書かなくてもスライスは完了する） |

**ジャーナルは 8 点目の成果物ではない。** 7 点セットは 7 点のまま。
完了条件（`check_deliverables.py`）にも入れない。
入れた瞬間に「書かされるもの」になり、学習資料としては死ぬ。

## いつ書くか（3 つだけ）

| 合図 | 具体例 |
|---|---|
| **判断点で選択肢を出した** | `option-first` を使った直後。設計の分かれ道を通った |
| **詰まって抜けた** | 最初の見立てが外れた。2 回以上やり直した |
| **節目の操作をした** | PR・マージ・ブランチ整理・リリース・設定の切り替え |
...
ジャーナルでは次の 3 つから **主題に合う 1〜2 枚** を選ぶ。

| 図 | 記法 | 向く回 |
|---|---|---|
| **判断の分岐図** | `flowchart TD` | 選択肢を比べて 1 つを採った回（既定。迷ったらこれ） |
| 時間の流れ図 | `sequenceDiagram` | 仮説 → 検証 → 結果を往復した回（デバッグ） |
| 理解の変化図 | `stateDiagram-v2` | 見立てが途中で入れ替わった回 |
...
- **採用: 7 点セットの外に置き、任意にした**
  —— 理由: 必須にすると `check_deliverables.py` の判定対象になり、
  「埋めるための文章」が書かれ始める。学習資料は義務化した瞬間に死ぬ
- **却下: 8 点目の必須成果物にする**
...
- 全反復で書く（形骸化する。書くのは上の 3 つの合図が出た回だけ）
