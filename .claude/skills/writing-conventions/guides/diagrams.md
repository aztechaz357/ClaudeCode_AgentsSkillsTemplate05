...
## 文書に置く図に draw.io を使わない

表 1 の図は **すべて文書の中にフェンスで書く** 。`.drawio` ファイルや
そこから書き出した画像を設計書・要求仕様書・ジャーナルに貼ることは
規約違反とする。Markdown 上でレンダリングされず、`check_diagrams.ps1` と
`check_mermaid_ids.ps1` の検証も通らないため、図がずれても誰も気づけない。

draw.io を使うのは **図を単体のファイルとして人に渡すとき** だけ
（受け取った人が手で編集する図・図形ライブラリが要る図・モックアップ）。
手順は `.claude/skills/drawio/SKILL.md` が正。
...
2. **ノード id はモジュールパス、表示名は日本語** ——
   ...
   **雛形の `{}` は `"…"`（表示名）の中だけに書く** —— id に書くと
   ...
   モジュールパスを要求するのは **層と依存の向きを描く図だけ**
   （比較の対象になるのがこの図だから）。判断の分岐図・状態遷移図など、
   実装と突き合わせない図の id は `observed` `h1` のような
   **ASCII の短い語** でよい。 **id が非 ASCII でないことと `{}` を
   書かないことは、どの図でも守る** （`check_mermaid_ids.ps1` が毎編集で見る）
