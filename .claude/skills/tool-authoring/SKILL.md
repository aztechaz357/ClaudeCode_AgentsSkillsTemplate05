...
| `build_usdm.py` | USDM 記法（`skills/usdm/`）の要求を検証し、要求ビューア（外部参照ゼロの自己完結 HTML）を生成する。理由の欠落・仕様 0 件・仕様番号の不整合・番号の重複・S## と REQ## の不一致を検出する | `<ツール実行コマンド> .claude/tools/build_usdm.py [--source <dir>...] [--out <html>] [--check]` | 違反なく生成できたら 0（`--check` では最新）。違反または STALE は 1。引数エラー・対象 0 件は 2 | slice-definition を使う工程すべて・doc-syncer（記録）・doc-reviewer・orchestrator（現在地の把握） |
