...
**まず動かしてみる場合は [QUICKSTART.md](QUICKSTART.md) から** ——
最初に打つコマンドと、場面別の台本（ツール 1 本・腰を据えた開発・
既存プロジェクトへの後付け）が載っています。
...
- **成果物は省きません** —— 要求仕様書・設計書・単体テスト・実装・
  統合テスト・テスト結果まとめ・マニュアルの **7 点を毎反復そろえます** 。
  省くのは分量であって、成果物の種類ではありません
- **テストは省きません** —— L1 でもテスト先行（Red 確認）と E2E 1 本は必須
- **手抜きは許し、隠すのは許しません** —— 仮実装は負債表に書けば
  正式に許可され、`/refactor` の工程で返します
- **クリーンアーキテクチャで書きます** —— 4 層 ＋ 依存性逆転。
  外部 I/O は契約（Port）越しに呼び、実装は 1 か所で注入します
- **判断は理由と却下案ごと残します** —— 設計書の「判断の記録」に、
  なぜそう考えたか・他の選択肢・メリデメを書きます
...
| 文書 | `README.md` 1 枚 | **8 点セット**（要求・設計・テスト仕様・テスト結果・マニュアル ほか） |
| 構造 | 1 ディレクトリ完結 | クリーンアーキテクチャ（4 層 + 契約） |
...
9. /status                          … 現在地を 1 画面で見る（いつでも）
10. /refactor                       … 負債がたまったら返す
11. /improve-process                … 数反復ごとにプロセス自体を改善する
```
