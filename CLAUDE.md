...
9. **目標成熟度を超えて作業しない**: L1 の反復で失敗経路を網羅する・
   ...
   **目標成熟度をプロンプトに必ず書く**
10. **8 点セットを 1 つも落とさない**: 要求仕様書・設計書・単体テスト・
...
12. **関与モードに従う**: プロファイルの「関与モード」を毎回読む。
    `伴走` のときは **決めてから説明しない** —— 段0・段1・段2・段3 では
    決める前に選択肢を出して待つ。サブエージェントを起動するときは
...
| **設計書を書く（薄い版・厚い版。4 種の図が必須）** | `.claude/skills/functional-design/` |
